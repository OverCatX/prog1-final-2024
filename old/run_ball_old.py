...
while (True):
    ...
    draw_border()
    for i in range(num_balls):
        ball_old.draw_ball(ball_color[i], ball_radius, xpos[i], ypos[i])
        ball_old.move_ball(i, xpos, ypos, vx, vy, dt)
        ball_old.update_ball_velocity(i, xpos, ypos, vx, vy, canvas_width, canvas_height, ball_radius)
    turtle.update()
